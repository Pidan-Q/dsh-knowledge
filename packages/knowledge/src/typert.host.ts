/**
 * knowledge 宿主面 Typert manifest（手写，对齐 @deepseek-ai/dsh-typert-generator
 * 的生成格式；迁入官方仓库后可切换为构建期生成产物）。
 *
 * 由 typert-loader 扫描包 `./typert` 导出注册到 `ctx.typert`，与
 * `KnowledgeRemoteService` 的 Cordis service key（`knowledge`）配对，
 * 浏览器端经 `ctx.remote.knowledge.*` 调用。所有 codec 均为 strict + zod v4。
 */

import { z } from 'zod'

/** KnowledgeEntry 的 wire 形状（与 shared/schema.ts 的 KnowledgeEntry 对齐）。 */
